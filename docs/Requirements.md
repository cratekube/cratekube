...
The following requirements are intended to provide guidance and structure for building the CrateKube system. Each requirement has been identified as an essential part of the architecture and should be incorporated to maximize value to administrators and customers.
...
As a user, I want my deployment to be accessible from outside the Cisco WAN, so that partners and customers may access my deployment from their homes or offices, because many of the users of the CrateKube system won't be within Cisco.

### AWS public cloud presence

![Generic badge](https://img.shields.io/badge/TECHNICAL-MVP-GREEN.svg)

As a user, I want CrateKube to have a public cloud presence on AWS, so that my resources can be accessed from outside of the Cisco network, because this open source project wouldn't be usable outside of Cisco if we target a Cisco-internal cloud.

### On-premise cloud

![Generic badge](https://img.shields.io/badge/BUSINESS-POSTMVP-brightgreen)

As a user, I want to be able to use CrateKube on-premise, so that my deployment is not unnecessarily exposed to the public Internet, because some use cases cannot be met in the public cloud.
...
As a user, I want real-time and historical logs to be automatically generated by my deployment, so that I may more easily meet compliance requirements and analyze logs for security, defect, or operational interests, because without logging I do not have a record of past events that have occurred in my deployment.
...
As a user, I want CrateKube to use event-driven architecture to manage my deployment's control plane, so I have a higher degree of resiliency and change management, because events promote decoupling and can be more easily queued, retried and recovered than direct service invocation.

## Secret management

![Generic badge](https://img.shields.io/badge/BUSINESS-MVP-GREEN.svg)

As a user, I want secret management integrated with my deployment, so that can more easily adhere to security best practices, because managing secrets is non-trivial and is a duplicated effort across teams

### Etcd Secret Migration

![Generic badge](https://img.shields.io/badge/TECHNICAL-POSTMVP-YELLOW.svg)

As a user, I want my secrets to be automatically migrated from etcd to the CrateKube solution, so that I do not have to recreate all of my secrets to use CrateKube's secret management feature, because a high barrier for adoption of this feature may stop users from using it

### Vault-based secret storage

![Generic badge](https://img.shields.io/badge/TECHNICAL-POSTMVP-YELLOW.svg)

As a user, I want my deployment's secrets stored in Vault, so that I have improved confidence in the security of the platform's secret management, because Vault is an industry standard for secrets management
...
As a user, I want access to NFS storage within my container, so that I may integrate with services and users that rely on NFS automounts, because NFS automounts are required by many of Cisco's use cases and are unavoidable for many user workflows

### Clustered file system

![Generic badge](https://img.shields.io/badge/TECHNICAL-POSTMVP-YELLOW.svg)

As a user, I want storage volumes to be backed by a clustered file system, so that I may share files between containers even when they are scheduled across hosts, because some deployments expect containers to be collocated on the same host.
...
As a user, I want automated management of my deployment via a rest-based API, so that I can extend and enhance the platform with integrations specific to my use-cases, because it is not possible for CrateKube's development team to develop to everyone's use case without contributions from the CrateKube community
...
As a user, I want machine learning to suggest best practices and fixes for common problems, so that I spend less time encountering and troubleshooting issues with my deployment, because ML can be used to cross reference a deployment's configuration and failures with observed-good configuration of similar deployments elsewhere on the platform.

### docker-compose v2 deployment spec

![Generic badge](https://img.shields.io/badge/BUSINESS-POSTMVP-brightgreen)

As a user, I want support for docker-compose v2 yml, so that I can deploy pods with the same configuration I use while developing locally, because Kurbenetes-yaml is verbose and difficult to understand.
...
As a user, I want the management of HTTPS certificates to be automated, so that I can more easily use applications secured with HTTPS, because without automated cert management certificate provisioning is tedious and error-prone.
